...
function ReputaAppContent() {
  ...
  // ✅ وظيفة الحفظ: تم تعديلها لتقبل عنواناً يدوياً (manualAddress) من خانة البحث
  const savePioneerToDatabase = async (user: any, manualAddress?: string) => {
    try {
      if (!user || user.uid === "demo") return;
      
      // نستخدم العنوان الذي أدخله المستخدم يدوياً إذا وجد، لأنه الأضمن (يبدأ بـ G)
      const finalWalletAddress = manualAddress || user.wallet_address || user.uid;
      ...
      console.log("Sync Success: ", finalWalletAddress);
    } catch (error) {
  ...
  useEffect(() => {
    const initApp = async () => {
      ...
      try {
        ...
        // نطلب اليوزر ونتحقق من صلاحية المحفظة
        const user = await authenticateUser(['username', 'wallet_address']).catch(() => null);
        
        if (user) {
          setCurrentUser(user);
          // حفظ أولي (قد يكون UID إذا لم يعطِ المتصفح الصلاحية فوراً)
  ...
  const handleWalletCheck = async (address: string) => {
    ...
    try {
      ...
      if (data && typeof data.reputaScore === 'number') {
        ...
        // ✅ هنا السر: نرسل العنوان الذي كتبه المستخدم (address) فوراً للقاعدة
